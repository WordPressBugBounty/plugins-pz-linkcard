...
<!-- 編集用フォーム -->
&nbsp;
<div class="pz-man-cache-editor-title"><div class="pz-man-cache-editor-icon"><?php echo __('&#x1f4dd;&#xfe0f;', TEXT_DOMAIN ); ?></div>&ensp;<div class="pz-man-cache-editor-text"><?php echo __('Cache Editor', TEXT_DOMAIN ); ?></div></div>
<div class="pz-man-cache-editor">
	<table class="wp-list-table"><!--  wp-list-table widefat fixed -->
		<tr>
			<td colspan="2" style="text-align: right;">
				<button type="submit" name="action" value="update" class="button button-primary button-large"><?php _e('Update', TEXT_DOMAIN ) ?></button>
				&emsp;
				<button type="submit" name="action" value="cancel" class="button button-large"><?php _e('Cancel', TEXT_DOMAIN ) ?></button>
			</td>
		</tr>
		<tr>
			<th><?php _e('ID', TEXT_DOMAIN ) ?></th>
		...
		<tr class="pz-debug-only">
			<th><?php _e('Registration Title', TEXT_DOMAIN ) ?></th>
		...
		<tr>
			<th><?php _e('Title', TEXT_DOMAIN ) ?> (<span style="text-decoration: underline;">2</span>)</th>
			<td><input name="data[title]" type="text" value="<?php echo esc_attr($data['title'] ); ?>" size="80" accesskey="2" /></td>
		</tr>
		<tr class="pz-debug-only">
			<th><?php _e('Modify Title', TEXT_DOMAIN ) ?></th>
		...
		<tr class="pz-debug-only">
			<th><?php _e('Registration Excerpt', TEXT_DOMAIN ) ?></th>
		...
		<tr>
			<th><?php _e('Excerpt', TEXT_DOMAIN ) ?> (<span style="text-decoration: underline;">3</span>)</th>
			<td><textarea name="data[excerpt]" cols="83" rows="5" wrap="soft" accesskey="3"><?php echo esc_attr($data['excerpt'] ); ?></textarea></td>
		</tr>
		<tr class="pz-debug-only">
			<th><?php _e('Modify Excerpt', TEXT_DOMAIN ) ?></th>
		...
		<tr>
			<th><?php _e('Character Set', TEXT_DOMAIN ) ?></th>
			<td><?php echo $data['regist_charset'].'&nbsp;'.__('->', TEXT_DOMAIN ); ?>&nbsp;<input name="data[charset]" type="text" value="edit" size="8" readonly="readonly"<?php if ($this->options['admin-mode'] ) { echo ' ondblclick="this.readOnly=false;"'; } ?> /></td>
		</tr>
		<tr>
			<th><?php _e('Thumbnail URL', TEXT_DOMAIN ) ?></th>
		...
		<tr>
			<th><?php _e('Favicon URL', TEXT_DOMAIN ) ?></th>
		...
		<tr id="update_result">
			<th><?php _e('Result Code', TEXT_DOMAIN ) ?></th>
			<td>
		...
		<tr>
			<th><?php _e('No Failure', TEXT_DOMAIN ) ?> (<span style="text-decoration: underline;">4</span>)</th>
			<td>
				<label><input name="data[no_failure]" type="checkbox" value="1" <?php checked(!empty($data['no_failure'] ? true : false ) ); ?> accesskey="4" /><?php _e('The result code is inaccessible but can actually be accessed.', TEXT_DOMAIN ); ?></label>
			</td>
		</tr>
		<tr>
			<th><?php _e('Post ID', TEXT_DOMAIN ) ?></th>
			<td>
		...
		<tr>
			<th><?php _e('SNS', TEXT_DOMAIN ) ?></th>
			<td>
				<?php _e('Tw', TEXT_DOMAIN ) ?>:<input name="data[sns_twitter]"  type="text" value="<?php echo $data['sns_twitter'];	 ?>" size="5" readonly="readonly"<?php if ($this->options['admin-mode'] ) { echo ' ondblclick="this.readOnly=false;"'; } ?> />
				<?php _e('fb', TEXT_DOMAIN ) ?>:<input name="data[sns_facebook]" type="text" value="<?php echo $data['sns_facebook']; ?>" size="5" readonly="readonly"<?php if ($this->options['admin-mode'] ) { echo ' ondblclick="this.readOnly=false;"'; } ?> />
				<?php _e('B!', TEXT_DOMAIN ) ?>:<input name="data[sns_hatena]"   type="text" value="<?php echo $data['sns_hatena'];	 ?>" size="5" readonly="readonly"<?php if ($this->options['admin-mode'] ) { echo ' ondblclick="this.readOnly=false;"'; } ?> />
				<?php _e('Po', TEXT_DOMAIN ) ?>:<input name="data[sns_pocket]"   type="text" value="<?php echo $data['sns_pocket'];	 ?>" size="5" readonly="readonly"<?php if ($this->options['admin-mode'] ) { echo ' ondblclick="this.readOnly=false;"'; } ?> />
			</td>
		</tr>
		<tr class="pz-debug-only">
			<th><?php _e('Last SNS Check', TEXT_DOMAIN ) ?></th>
			<td><input name="data[sns_time]" type="text" value="<?php echo $data['sns_time']; ?>" size="8" readonly="readonly"<?php if ($this->options['admin-mode'] ) { echo ' ondblclick="this.readOnly=false;"'; } ?> /><?php echo ' '.$this->pz_Date(DATETIME_FORMAT, $data['sns_time'] ); ?></td>
		</tr>
		<tr class="pz-debug-only">
			<th><?php _e('Next SNS Check', TEXT_DOMAIN ) ?></th>
			<td><input name="data[sns_nexttime]" type="text" value="<?php echo $data['sns_nexttime']; ?>" size="8" readonly="readonly"<?php if ($this->options['admin-mode'] ) { echo ' ondblclick="this.readOnly=false;"'; } ?> /><?php echo ' '.$this->pz_Date(DATETIME_FORMAT, $data['sns_nexttime'] ); ?></td>
		</tr>
		<tr class="pz-debug-only">
			<th><?php _e('Registration Character Set', TEXT_DOMAIN ) ?></th>
		...
		<tr class="pz-debug-only">
			<th><?php _e('Registration Date', TEXT_DOMAIN ) ?></th>
		...
		<tr>
			<th><?php _e('Registration Date', TEXT_DOMAIN ) ?></th>
			<td><?php echo ' '.$this->pz_Date(DATETIME_FORMAT, $data['regist_time'] ); ?></td>
		</tr>
		<tr class="pz-debug-only">
			<th><?php _e('Registration Result Code', TEXT_DOMAIN ) ?></th>
			<td>
		...
		<tr class="pz-debug-only">
			<th><?php _e('Last Alive Check', TEXT_DOMAIN ) ?></th>
			<td><input name="data[alive_time]" type="text" value="<?php echo $data['alive_time']; ?>" size="8" readonly="readonly"<?php if ($this->options['admin-mode'] ) { echo ' ondblclick="this.readOnly=false;"'; } ?> /><?php echo ' '.$this->pz_Date(DATETIME_FORMAT, $data['alive_time'] ); ?></td>
		</tr>
		<tr class="pz-debug-only">
			<th><?php _e('Next Alive Check', TEXT_DOMAIN ) ?></th>
			<td><input name="data[alive_nexttime]" type="text" value="<?php echo $data['alive_nexttime']; ?>" size="8" readonly="readonly"<?php if ($this->options['admin-mode'] ) { echo ' ondblclick="this.readOnly=false;"'; } ?> /><?php echo ' '.$this->pz_Date(DATETIME_FORMAT, $data['alive_nexttime'] ); ?></td>
		</tr>
		<tr class="pz-debug-only">
			<th><?php _e('Result Code of Alive Check', TEXT_DOMAIN ) ?></th>
			<td>
